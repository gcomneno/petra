...
def test_prebranch_runner_can_use_coupling_aware_contextual_selector():
    state = _make_state()

    selector = make_contextual_branch_selector(
        context_score_by_coupling_weighted_hint_density,
        maximize=True,
        selector_name="prefer-coupled-tight-slot",
        context_builder=build_coupling_aware_branch_selection_context,
    )
