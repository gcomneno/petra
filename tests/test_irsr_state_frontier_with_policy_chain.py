...
def test_run_residual_state_frontier_until_quiescence_with_policy_chain_refines_then_promotes():
    state = _load_state()
    ...
    assert result["idle"] == []
    assert result["trace"] == [
        {"step": 1, "action": "refine", "refiner": "_seed_b_with_113"},
        {"step": 2, "action": "promote"},
    ]
    assert result["termination_reason"] == "frontier-exhausted"


def test_run_residual_state_frontier_until_quiescence_with_policy_chain_quiesces_without_progress():
    state = _load_state()
    state = refine_residual_state_slot_candidates(state, "a", [101])
