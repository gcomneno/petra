...
def test_pet_builder_e2e_from_real_cli_canonical_build_payload_for_30(tmp_path: Path) -> None:
    import os
    ...
    assert cli_payload["input_n"] == 30
    assert cli_payload["target_n"] == 30
    assert cli_payload["target_generator"] == 30

    support = _run_support(cli_payload, tmp_path)
    assert support["source_schema"] == "pet-build-from-int-canonical-cli-v0"
    assert support["builder_readiness"] == "ready"
    assert support["unknown_block_count"] == 0
    assert support["known_block_count"] == 3
...
def test_pet_builder_e2e_from_real_cli_partial_build_payload_for_20(tmp_path: Path) -> None:
    import os
    ...
    assert support["builder_readiness"] == "ready"
    assert support["unknown_block_count"] == 0
    assert support["known_block_count"] == 2
    ...
    execution = _run_execute(plan, tmp_path)
    assert execution["execution_status"] == "executed"
    assert execution["materialized_artifact_count"] == 2
    assert execution["final_build_output"]["build_status"] == "built"
    assert execution["final_build_output"]["built_pet_object"]["assembly_status"] == "assembled"
    assert execution["final_build_output"]["built_pet_object"]["component_count"] == 2


def test_pet_builder_e2e_from_real_cli_canonical_build_payload_for_210(tmp_path: Path) -> None:
    import os
    ...
    assert "schema" not in cli_payload
    assert cli_payload["input_n"] == 210
    assert cli_payload["target_n"] == 210
    assert cli_payload["target_generator"] == 210

    support = _run_support(cli_payload, tmp_path)
    assert support["source_schema"] == "pet-build-from-int-canonical-cli-v0"
    assert support["builder_readiness"] == "ready"
    assert support["unknown_block_count"] == 0
    assert support["known_block_count"] == 4
    ...
    execution = _run_execute(plan, tmp_path)
    assert execution["execution_status"] == "executed"
    assert execution["materialized_artifact_count"] == 4
    assert execution["final_build_output"]["build_status"] == "built"
    assert execution["final_build_output"]["built_pet_object"]["assembly_status"] == "assembled"
    assert execution["final_build_output"]["built_pet_object"]["component_count"] == 4


def test_pet_builder_e2e_from_real_cli_partial_build_payload_for_18(tmp_path: Path) -> None:
    import os
    ...
    assert cli_payload["schema"] == "pet-build-from-int-v2"
    assert cli_payload["input_n"] == 18
    assert cli_payload["target_generator"] == 12
    assert cli_payload["build_status"] == "partial"
