...
def test_cli_builder_from_factors_json_smoke(tmp_path) -> None:
    path = tmp_path / "factors.json"
    path.write_text(json.dumps({"factors": [[2, 1], [3, 1], [5, 1]]}), encoding="utf-8")

    out = _run_cli("builder-from-factors", str(path), "--json")
    payload = json.loads(out)

    assert payload["schema"] == "pet-builder-from-factors-v0"
