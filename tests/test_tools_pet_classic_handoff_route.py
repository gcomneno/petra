...
def test_classic_handoff_route_reports_policy_without_legacy_fallback() -> None:
    ...
    assert (
        "reason = balanced flat PET border has a DROP lens transition; run conservative classic residual probe"
        in output
    )
    assert (
        "suggested_command = python -m pet.cli opaque-probe 3027009081 --trial-limit 20"
        in output
    )
    assert "route_kind = fork-follow-rescan" not in output
    assert (
        "claim = PET classic handoff route only; classic verification required"
        in output
    )
...
def test_classic_handoff_route_supports_atomic_exact_primality_policy() -> None:
    ...
    assert (
        "reason = atomic PET shape; run minimal classic residual/primality probe"
        in output
    )
    assert (
        "suggested_command = python -m pet.cli opaque-probe 10007 --trial-limit 2"
        in output
    )
    assert "route_kind = fork-follow-rescan" not in output
...
def test_classic_handoff_route_supports_narrow_deep_power_policy() -> None:
    ...
    assert (
        "reason = narrow deep PET shape; run minimal classic check for repeated small factors"
        in output
    )
    assert (
        "suggested_command = python -m pet.cli opaque-probe 65536 --trial-limit 2"
        in output
    )
    assert "route_kind = fork-follow-rescan" not in output
...
def test_classic_handoff_route_supports_wide_exact_backbone_policy() -> None:
    ...
    assert (
        "reason = wide exact PET shape; run classic probe bounded by selected backbone order"
        in output
    )
    assert (
        "suggested_command = python -m pet.cli opaque-probe 30030 --trial-limit 6"
        in output
    )
    assert "route_kind = fork-follow-rescan" not in output
...
def test_classic_handoff_route_supports_near_shape_operator_policy() -> None:
    ...
    assert (
        "reason = near PET shape; run classic probe bounded by selected operator neighborhood"
        in output
    )
    assert (
        "suggested_command = python -m pet.cli opaque-probe 9999999999 --trial-limit 5"
        in output
    )
    assert "route_kind = fork-follow-rescan" not in output
...
def test_classic_probe_policy_maps_pet_shape_diagnostics() -> None:
    ...
    spec = importlib.util.spec_from_file_location(
        "pet_classic_handoff_route", module_path
    )
    ...
    assert (
        module.classic_probe_policy("complex-border") == "complex-border-route-needed"
    )
...
def test_classic_handoff_route_can_use_active_blade_window_for_large_input() -> None:
    ...
    assert (
        "suggested_command = python -m pet.cli opaque-probe 387456687301039324825975283416 --trial-limit 20"
        in output
    )


def test_classic_handoff_route_reports_decimal_rigid_border_fields_from_proposal_file(
    tmp_path,
) -> None:
    proposal_file = tmp_path / "proposal.txt"
    ...
    assert "classic_probe_policy = complex-border-route-needed" in output
    assert (
        "route_note = decimal rigid border detected; route-only benchmark may be expensive"
        in output
    )
    assert "route_status = available" in output
    assert "route_kind = balanced-flat-border-lens-drop-classic-probe" in output
...
def test_classic_handoff_route_reports_atomic_leaf_policy() -> None:
    ...
    assert (
        "auto_same_shape_scan_skip_reason = route-policy-leaf-primality-route" in output
    )
...
def test_classic_handoff_route_summary_reports_candidate_partial_factorization() -> (
    None
):
    output = run_tool_with_args(
        24680,
    ...
    assert (
        "summary_route_final_status = partial-factorization-by-pet-candidate" in output
    )
...
def test_shape_family_route_registry_maps_known_shapes() -> None:
    ...
    assert module.shape_family_support_status_for_signature("[[]]") == "supported"

    assert module.shape_family_class_for_signature("[[], []]") == "semiprime-flat"
    assert module.shape_family_route_for_signature("[[], []]") == "same-shape-flat"
    assert module.shape_family_support_status_for_signature("[[], []]") == "supported"
    ...
    assert (
        module.shape_family_support_status_for_signature("[[], [], []]") == "recognized"
    )

    assert module.shape_family_class_for_signature("[[[]]]") == "narrow-deep-chain"
    assert module.shape_family_route_for_signature("[[[]]]") == "power-like-local-check"
...
def test_classic_handoff_route_reports_flat_k_scan_required_for_no_grip_flat_k() -> (
    None
):
    output = run_tool_with_args(
        1001,
