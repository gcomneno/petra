#!/usr/bin/env python3
"""Deterministic evidence runner for PETRA VISION Gate 3 G3-S2."""
...
s2 = _load_module(
    S2_TOOL_PATH,
    "_petra_vision_gate3_s2_evidence_s2",
)

s01 = _load_module(
    S01_TOOL_PATH,
    "_petra_vision_gate3_s2_evidence_s01",
)

width4 = _load_module(
    WIDTH4_TOOL_PATH,
    "_petra_vision_gate3_s2_evidence_width4",
)


if s2.PROTOCOL_ID != EXPECTED_S2_PROTOCOL_ID:
    raise RuntimeError(
        "evidence runner requires frozen G3-S2 protocol"
    )
...
def compare_partitions(
    ...
    reference_groups = tuple(
        tuple(group)
        for group in reference_summary[
            "collision_shape_codes"
        ]
    )
    ...
    reference_pairs = _collision_pairs_from_groups(
        reference_groups
    )
    ...
    split = sorted(
        reference_pairs - candidate_pairs
    )
    ...
    return {
        "distinct_signature_delta": (
            ...
            - int(reference_summary[
                "distinct_signatures"
            ])
        ),
        "reference_collision_pair_count": len(
            reference_pairs
        ),
...
def analyze_corpus(
    ...
    for substrate in SUBSTRATES:
        ...
        result: dict[str, object] = {
            "local_unit_count_distribution": {
                str(count): frequency
                for count, frequency in sorted(
                    unit_distribution.items()
                )
            },
            "s1_global_operator_spectrum": (
                s1_result
            ),
            "s2_local_spectrum_multiset": (
                s2_result
            ),
            "s1_to_s2": compare_partitions(
                s1_result["summary"],
                s2_result["summary"],
            ),
        }
        ...
        if substrate == "F1":
            ...
            intrinsic_result = _analyze_channel(
                codes=codes,
    ...
    substrate_results["L1"] = {
        "status": s2.NOT_APPLICABLE,
    }
...
def build_evidence(
    ...
    return {
        ...
        "s2_protocol_id": s2.PROTOCOL_ID,
        "s0_s1_protocol_id": s01.PROTOCOL_ID,
        "source_sha256": {
            "s2_tool": _sha256_file(
                S2_TOOL_PATH
            ),
            "s0_s1_tool": _sha256_file(
                S01_TOOL_PATH
            ),
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate exact Gate 3 G3-S2 evidence "
