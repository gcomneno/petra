#!/usr/bin/env python3
"""Deterministic evidence runner for PETRA VISION Gate 3 G3-H1."""
...
h1 = _load_module(
    H1_TOOL_PATH,
    "_petra_vision_gate3_h1_evidence_tool",
)

s01 = _load_module(
    S01_TOOL_PATH,
    "_petra_vision_gate3_h1_evidence_s01",
)

width4 = _load_module(
    WIDTH4_TOOL_PATH,
    "_petra_vision_gate3_h1_evidence_width4",
)


if h1.PROTOCOL_ID != EXPECTED_H1_PROTOCOL_ID:
    raise RuntimeError(
        "evidence runner requires frozen G3-H1 protocol"
    )
...
if h1.TRUNCATIONS != (
    ...
    raise RuntimeError(
        "evidence runner requires frozen H1 truncation ladder"
    )
...
def _assert_h1_is_s1_compression(
    ...
    *,
    ...
    if comparison[
        ...
        raise RuntimeError(
            "H1 split an S1 collision: "
            f"{substrate} {truncation}"
        )


def _assert_ladder_refinement(
    comparison: dict[str, object],
    *,
    substrate: str,
    lower: str,
    ...
    if comparison[
        ...
        raise RuntimeError(
            "lower H1 truncation refined "
            "a higher truncation: "
            f"{substrate} {lower} vs {higher}"
        )
...
def analyze_corpus(
    ...
    for substrate in s01.SUBSTRATES:
        ...
        for index, geometry in enumerate(
            ...
            orders.append(
                h1.operator_order_from_spectrum(
                    spectrum
                )
            )
            ...
            for name, _max_moment in h1.TRUNCATIONS:
                moments[name].append(
                    signatures[name]
                )
        ...
        for name, _max_moment in h1.TRUNCATIONS:
            ...
            if (
                ...
                raise RuntimeError(
                    "#23/#41 separated under H1 "
                    f"for {substrate} {name}"
                )
        ...
        f1_m4_matches_s1 = None

        if substrate == "F1":
            all_order_at_most_four = all(
                order <= 4
                for order in orders
            )
            ...
            f1_m4_matches_s1 = {
                "all_operator_orders_at_most_four": (
                    all_order_at_most_four
                ),
                "same_partition": (
                    m4_comparison[
                        "split_pair_count"
            ...
            if (
                all_order_at_most_four
                and not f1_m4_matches_s1[
                    "same_partition"
                ]
            ):
                raise RuntimeError(
                    "F1 M4 failed degree<=4 "
                    "Newton-identity sanity check"
                )

        substrate_results[
            substrate
        ] = {
            "operator_order_distribution": {
                str(order): count
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate exact Gate 3 G3-H1 evidence "
